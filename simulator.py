...
if __name__ == "__main__":
    ...
    model_bounds = model.ModelBounds((5,5), (3,3), 4, 0.5, 4)
    #model = model.generate_random_model(model_bounds, rng)
    model = model.generate_special_model(model_bounds, rng)
    ...
    optimal_agent = agent.PolicyAgent((5,5), 3,3, policy, rng)
    rc_agent = agent.RC_Agent((5,5),3,3, model_bounds, rng)
    abl_agent = agent.RC_Agent((5,5),3,3, model_bounds, rng, True)
    ...
    sim = Simulator(model, optimal_agent, optimal_observer, rng, rand_rew=False)
    rc_sim = Simulator(model, rc_agent, rc_observer, rng, rand_rew=False)
    abl_sim = Simulator(model, abl_agent, abl_observer, rng, rand_rew=False)
