...
class ParameterEstimator:
    def __init__(self, model_bounds):
        ...
        self.positive_sojourn_times = [[[] for j in range(model_bounds.n_customer_levels)] for i in range(model_bounds.n_states)]
        self.negative_sojourn_times = [[[] for j in range(model_bounds.n_server_levels)] for i in range(model_bounds.n_states)]

        self.positive_clock = [[0 for j in range(model_bounds.n_customer_levels)] for i in range(model_bounds.n_states)]
        self.negative_clock = [[0 for j in range(model_bounds.n_server_levels)] for i in range(model_bounds.n_states)]

    def observe(self, state, action, transition_type, time_elapsed):
    ...
    def sojourn_time_estimate(self, state, confidence_param, is_positive):
        ...
        for i, stime in enumerate(times):
            truncation = math.sqrt(2*(i+1)/(math.pow(min_rate,2)*max(math.log(2*self.model_bounds.n_states/confidence_param),0.00001)))
            if stime <= truncation:
                acc += stime
        return acc/ct
    ...
    def sojourn_time_epsilon(self, state, confidence_param, is_positive):
        ...
        inner_term = (2/max(1, ct))*max(math.log(2*self.model_bounds.n_states/confidence_param),0.00001)
